...
def get_model(config):
    ...
    category_feature = utils.id_input_processing_concat(category_feature_input, config)
    ...
    # all_feature = layers.Concatenate(axis=-1)([sequence_feature, dense_feature])
    sequence_feature_dnn = layers.Dense(256, activation=layers.ELU())(sequence_feature)
    all_feature = layers.Concatenate(axis=-1, name='simulator_obs')(
        [sequence_feature_dnn, dense_feature, category_feature]
    )
    output = layers.Dense(22, activation='softmax', name='simulator_reward')(all_feature)
